...
const carouselHTMLTemplate = (ctx) => html`
   ...
   <div class="carousel-container">
      ${ctx.articleData.map(([img, caption, isVisible], index) => {
         return html`
            <article style=${isVisible ? 'display: block' : 'display: none'} class="carousel-slide fade">
...
class Carousel extends HTMLElement {
   ...
   get imageUrls() {
      return [...this.attributes].filter((a) => a.name.includes('image-url')).map((a) => a.value);
   }

   get captions() {
      return [...this.attributes].filter((a) => a.name.includes('caption')).map((a) => a.value);
   }

   get articleData() {
      return this.imageUrls.reduce((data, img, index) => {
