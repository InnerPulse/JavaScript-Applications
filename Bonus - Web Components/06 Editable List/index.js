...
import { repeat } from '//unpkg.com/lit-html/directives/repeat?module';
...
const editableListTemplate = (ctx) => html`
   <style>
      ...
      .editable-list-header {
         ...
         text-align: center;
         color: white;
         box-shadow: inset 0 -2px 3px 2px rgba(0, 0, 0, 0.4), 0 2px 2px 2px rgba(0, 0, 0, 0.4);
      }
      .editable-list {
         padding-left: 0;
      }
         ...
         ${repeat(
            ...
            (item, index) => html`
               <li>
                  ...
                  <button @click=${ctx.deleteItemHandler} data-target=${index} class="editable-list-remove-item icon">&ominus;</button>
               </li>
            `
         )}
      </ul>
...
class EditableList extends HTMLElement {
   ...
   addItemClickHandler() {
      if (this.state.addItemValue !== '') {
         this.state.listItems = this.state.listItems.concat(this.state.addItemValue);
